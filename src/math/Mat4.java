...
public class Mat4 extends BufferableData<FloatBuffer> {
	...
	public static Mat4 getScale(float x, float y, float z) {
		...
		res.setColumn(0, new Vec4(x, 0, 0, 0));
		res.setColumn(1,  new Vec4(0, y, 0, 0));
		res.setColumn(2, new Vec4(0, 0, z, 0));
		res.setColumn(3, new Vec4(0, 0, 0, 1));
	...
	public static Mat4 getRotate(float angDeg, Vec3 unitDirection) {
		float angRad = (float) Math.toRadians(angDeg);
		float cos = (float) Math.cos(angRad);
		float sin = (float) Math.sin(angRad);
		
		float Ax = unitDirection.x;
		float Ay = unitDirection.y;
		float Az = unitDirection.z;
		
		Mat4 res = new Mat4();
		
		// X column
		res.matrix[0] = cos + (1 - cos) * ((float) Math.pow(Ax, 2));
		res.matrix[1] = (1 - cos) * Ax * Ay + sin * Az;
		res.matrix[2] = (1 - cos) * Ax * Az - sin * Ay;
		
		// Y column
		res.matrix[4] = (1 - cos) * Ax * Ay - sin * Az;
		res.matrix[5] = cos + (1 - cos) * ((float) Math.pow(Ay, 2));
		res.matrix[6] = (1 - cos) * Ay * Az + sin * Ax;
		
		// Z column
		res.matrix[8] = (1 - cos) * Ax * Az + sin * Ay;
		res.matrix[9] = (1 - cos) * Ay * Az - sin * Ax;
		res.matrix[10] = cos + (1 - cos) * ((float) Math.pow(Az, 2));
